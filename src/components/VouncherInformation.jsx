...
const VouncherInformation = () => {
  ...
  const { data, error, isLoading } = useSWR(
    import.meta.env.VITE_API_URL + "/products?sort_by=id&sort_direction=desc&limit=100",
    fetcher
  );
  ...
  const onSubmit = async (VouncherData) => {
    const current = data?.data.find((el) => el.id == VouncherData.id);
console.log(current)
    const currentArray = {
      id:current.id,
    ...
    <div>
      <div className="   print:hidden">
        <form
          ...
          className=" mx-auto grid grid-cols-4 gap-4"
        >
          {/* select in first bd */}
      ...
      > */}
          ...
          <div className="col-span-1">
            <button
              type="submit"
              className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg w-full text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800"
            >
              Add
            </button>
          </div>
